...
import com.example.administrator.jingdong.view.ShouYeView.NoticeView;

import java.util.ArrayList;
import java.util.List;

public class SY_Myadapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
    ...
    public SY_Myadapter(Context context, List<Jiugongge_bean.DataBean> data , Shouye_bean shouye_bean) {
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, int position) {
        ...
        if (holder instanceof Miaosha){
            ...
            final Miaoshaadapter miaosha=new Miaoshaadapter(context, miaoshalist);
            ...
            for (int i=0;i<miaoshalist.size();i++){
                listM.add(miaoshalist.get(position).getImages());
            }
            miaosha.setOnItemClickListener(new Miaoshaadapter.OnItemClickListener() {
                @Override
                public void itemClick(View v, int position) {
                    ...
                    //截取第一张
                    String[] split = miaoshalist.get(position).getImages().split("\\|");
